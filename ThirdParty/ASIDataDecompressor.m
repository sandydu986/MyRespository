...
+ (BOOL)uncompressDataFromFile:(NSString *)sourcePath toFile:(NSString *)destinationPath error:(NSError **)err
{
    ...
    while ([decompressor streamReady]) {
		...
		// Make sure nothing went wrong/*为ios7修改——if ([inputStream streamStatus] == NSStreamEventErrorOccurred) */
		...
		// Make sure nothing went wrong/*为ios修改——*if ([inputStream streamStatus] == NSStreamEventErrorOccurred)
		if ([inputStream streamStatus] == (NSStreamStatus)NSStreamEventErrorOccurred) {
			if (err) {
...
+ (NSError *)inflateErrorWithCode:(int)code/*为ios7修改%hi为%d*/
{
